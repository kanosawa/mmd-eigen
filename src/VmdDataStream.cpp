	...
	const vector<BoneInfo>& boneInfoList, const Eigen::Vector3f& parentBeforePos)
{
	...
	for (unsigned int i = 0; i < childBoneIndices.size(); ++i) {
		...
		bones[childBoneIndex].setPosition(bones[parentBoneIndex].getQuaternion() *
			(bones[childBoneIndex].getPosition() - parentBeforePos + boneInfoList[childBoneIndex].shift) +
...
bool VmdDataStream::calcStream(BoneStream& boneStream, VertexStream& vertexStream, const vector<Bone>& initialBones, const vector<Vertex>& initialVertices)
{
	// ボーン情報リストをフレームごとに分割
